...
def filter_videos(file, mids, mid2class, wfile):
    # 筛掉audioset里面原始的标注到25个类，并且按照AVVP的anno文件重写文件
    mids = set(mids)
...
def check_inconsistence(avvp_file, total_file):
    # check哪些视频的标注与原标注不一致,共有439个视频的标注与原标注不完全一致
...
def check_speech(avvp_file):
    # 检查AVVP中多少视频的标注只有speech这个类别，共有9个视频

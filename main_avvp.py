...
# percent = 1
# low_confidence_file = f"data/low_confidence_{percent}%.txt"


def check_low_confidence_resp(F_seg_a, F_seg_v, F_seg, F_seg_av, F_event_a, F_event_v, F_event, F_event_av, val_loader, percent, file):
    ...
    with open(file, 'w', encoding='utf-8') as f:
        ...
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(seg_a_idx, F_seg_a[seg_a_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("seg_v_idx\tF_seg_v\n")  # segment_visual
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(seg_v_idx, F_seg_v[seg_v_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("seg_idx\tF_seg\n")  # segment
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(seg_idx, F_seg[seg_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("seg_av_idx\tF_seg_av\n")  # segment_audio_visual
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(seg_av_idx, F_seg_av[seg_av_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("event_a_idx\tF_event_a\n")  # event_audio
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(event_a_idx, F_event_a[event_a_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("event_v_idx\tF_event_v\n")  # event_visual
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(event_v_idx, F_event_v[event_v_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("event_idx\tF_event\n")  # event
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(event_idx, F_event[event_idx])]
        f.write("\n".join(tmp))
        f.write("\n")

        f.write("event_av_idx\tF_event_av\n")  # event_audio_visual
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(event_av_idx, F_event_av[event_av_idx])]
        f.write("\n".join(tmp))
        f.write("\n")


def check_low_confidence_togh(F_seg_a, F_seg_v, F_seg, F_seg_av, F_event_a, F_event_v, F_event, F_event_av, val_loader, percent, file):
    F_seg_a = np.array(F_seg_a)
    F_seg_v = np.array(F_seg_v)
    F_seg = np.array(F_seg)
    F_seg_av = np.array(F_seg_av)
    ...
    with open(file, 'w', encoding='utf-8') as f:
        ...
        tmp = ["\t".join([str(i), str(round(s, 2))]) for i, s in zip(idxes, scores[idxes])]
        f.write("\n".join(tmp))
